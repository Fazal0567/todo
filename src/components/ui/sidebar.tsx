...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn("hidden md:flex flex-col h-screen fixed z-10", className)}
        data-state={state}
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex items-center p-2 h-14 border-b", state === 'collapsed' ? 'justify-center' : '', className)}
      {...props}
    />
  )
})
  ...
  (
    ...
    const buttonContent = (
      <>
        {React.Children.map(children, (child) => 
          React.isValidElement(child) && child.type === 'span' && state === 'collapsed' ? null : child
        )}
      </>
    );
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant, size }), state === 'collapsed' ? '!size-8 !p-2 justify-center' : '', className)}
        {...props}
      >
        {buttonContent}
      </Comp>
